...
| `20-architecture/02-solution-structure.md` §6 | Replace **FluentAssertions with Shouldly 4.3.0** `[DEC-118]`. 8.x ships an Xceed Community License for **non-commercial use only**, and 7.2.0 — the last `Apache-2.0` release — is the end of that line. The table names the library with no version because it was written while it was still open source |
