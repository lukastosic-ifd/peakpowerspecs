...
| F01-R18 | ⚠ **Amended 2026-09-18 to match [DEC-152]'s model — the actor is a customer admin, not an employee.** A **customer admin** resends a pending invitation from the member-management surface **[F01-R21]** via `POST /api/v1/company/invitations/{id}/resend`, which mints a **fresh token and restarts the 14-day expiry, invalidating the previous link** (an in-place reissue on the existing row, so the old link's hash no longer matches). It works whether the invitation is still pending or has already expired, and is refused on one that was already accepted. ⚠ **Extended 2026-09-18 by [DEC-154]:** the back office now resends too — `POST /api/v1/customers/{customerId}/invitations/{id}/resend`, reusing the same shared reissue core — because it now **issues** invitations of its own (it gained `INSERT` + a column-scoped `UPDATE (token_hash, expires_at)` on `customer.customer_invitation`). The earlier note that "the back office has no part in this / holds SELECT-only access" is withdrawn. Original text (pre-[DEC-152], when accounts were employee-created): *"An employee can resend an invitation and can reissue one that has expired; the previous link is invalidated."* | Must |
...
| ID | Requirement | MoSCoW |
| --- | --- | :--: |
| F01-R42 | A customer company carries a **`four_eyes_enabled`** flag, default **off** **[DEC-71]**. It is set and cleared by a **PeakPower employee**, consistent with what survives of **[DEC-16]**: the customer decides internally who is trusted, PeakPower administers the record. There is **no amount threshold** of any kind, and no per-action override — the mode is all five actions or none. ⚠ **Narrowed 2026-09-10 by [DEC-152]: read as four, not five.** Membership changes bypass four-eyes entirely **[F01-R49]**; the mode's own closed set is now add a bank account, deactivate a bank account, execute a trade and withdraw funds. | Must |
...
| F01-R50 | While four-eyes is on, any change that would leave the company with **fewer than two `ACTIVE` admin accounts** is **refused** — deactivating the second-to-last admin **[F01-R16]**, or ~~clearing the admin flag on it~~ ⚠ **Reworded 2026-09-10 by [DEC-152]** — demoting an admin's `membershipRole` to `trader` or `viewer` **[F01-R47]**. The routes out are to appoint another admin first, or to switch four-eyes off, which is itself an audited employee action **[F01-R42]**. ⚠ **Recorded cost:** a company that loses an admin to illness or departure is blocked on bank-account changes, ~~user additions, ~~trades and withdrawals until PeakPower intervenes. ⚠ **"user additions" removed 2026-09-10 by [DEC-152]** — membership changes bypass four-eyes entirely **[F01-R49]**, so a below-floor company is not blocked from adding a member; adding or promoting an admin is in fact how the two-admin floor is restored. That is deliberate. The alternative — silently degrading to single approval when a company is short-staffed — removes the control exactly when it matters most. | Must |
...
14. **Four-eyes is a mode, not a threshold** **[DEC-71]**. It is on or off per customer company and,
    when on, it applies to four actions at any size: add a bank account, deactivate a bank account,
    ~~add a user~~, execute a trade, withdraw funds. ⚠ **Narrowed 2026-09-10 by [DEC-152]: adding a
    member bypasses four-eyes entirely** **[F01-R49]** — four actions remain, not five. **Deposits are
    out** — one person can wire money or use iDEAL unaided **[DEC-106]**, so an approval step there
    would control nothing. ⚠ This **replaces [DEC-33]**'s value threshold, and with it the threshold
    reference table **[F05-R50]**.
...
| `customer_account` | id, ~~customer_id~~ (⚠ **dropped 2026-09-10 by [DEC-152]** — an account's business now lives on `customer_membership`, not a column here **[F01-R13]**), **username**, first_name, last_name, **job_title**, phone, email, status (now including `PENDING_APPROVAL` **[F01-R15]**), ~~`is_admin` [DEC-71]~~ (⚠ **dropped 2026-09-10 by [DEC-152]** — replaced by **`membershipRole`** on `customer_membership`, one per business **[F01-R47]**), **`password_hash` + `security_stamp` [DEC-113], [DEC-117]**, ~~external_subject_id~~ (⚠ dead column, **[DEC-119]**), created_by_employee, created_at, last_login_at |
| `approval_request` | **New [DEC-71]**. id, customer_id, action (`ADD_BANK_ACCOUNT` \| `DEACTIVATE_BANK_ACCOUNT` \| `TRADE` \| `WITHDRAWAL` — ⚠ **`ADD_USER` removed 2026-09-10 by [DEC-152]**, which takes membership changes out of four-eyes **[F01-R49]**), subject_id, requested_by_account_id, requested_at, decided_by_account_id, decided_at, outcome (`PENDING` \| `APPROVED` \| `DECLINED`), reason. `decided_by_account_id <> requested_by_account_id` is a database constraint, not only an application check **[F01-R48]**. The trade and withdrawal rows are written by [F05](F05-energy-block-trading.md) and [F06](F06-wallet-and-ledger.md); the table is shared so that one screen shows a company everything waiting on it |
...
| `onboarding_application` | **New [DEC-113]**. The nine-step wizard's draft before a company exists: reference, status, the applicant's name, email and Argon2id password hash, then every later step's answers as nullable columns, `signatories` as `jsonb`, a hashed six-digit sign code with an attempt counter, and a nullable `customer_id` / `account_id` written only at signing. ⚠ **The applicant's columns are `NOT NULL` and every later answer is nullable because the account step is step 1** **[DEC-145]** — `POST /onboarding/applications` is that step's own call and is what creates the row, so every other answer arrives by `PATCH` against an application that already exists. The company steps are **2** and **3**, as the contract has numbered them throughout |
| `refresh_token`, `password_reset_token` | **New [DEC-117]**, **[DEC-113]**. Both store the token **hashed**, and the refresh chain records what replaced each token so a theft revokes the whole chain. ⚠ **Corrected 2026-09-10 by [DEC-152]: the two no longer share one scoping story.** `refresh_token` gains its own **`customer_id`** column (migration 15) — added nullable, backfilled, then set `NOT NULL`, indexed, and policed by `refresh_token_tenant_isolation` as `customer_id = app.customer_id` — so a session is tied to the business it was issued for, not merely to the account. `password_reset_token` keeps the original story: it is scoped by **account** only, and its row-level policy reaches the business through an `EXISTS` against `customer_membership` on `customer_account_id`, not through a column of its own |
...
- ~~Any permission model inside a company **[DEC-16]**.~~ ⚠ **Amended 2026-08-19 by [DEC-71]** — ~~one
  ...
  hierarchy~~. ⚠ **Reversed 2026-09-10 by [DEC-152]** — `is_admin` is dropped, and there **is** a role
  hierarchy: a **`membershipRole`** of `admin`, `trader` or `viewer` on `customer.customer_membership`,
  one per business, and `admin` gates real writes — invitations, role changes, removals and
  entitlements **[F01-R13]**, **[F01-R47]**. Still out: no per-metering-point scoping, no per-amount
  limits, no customer-defined roles — the three roles are fixed, not created by the customer.
