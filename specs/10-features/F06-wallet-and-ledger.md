...
| [OQ-110] | Registered by **[DEC-161]**. **[F06-R17]** (Must) requires a wallet's active reservations to be listed with trade links and ages; `GET /wallet` returns the aggregate `reservedAmount` only, with no per-reservation breakdown. The redesigned Balance page's Reserved definition falls back to naming the known withdrawal holds and folding the rest into "and accepted trades" until it lands |
| [OQ-111] | Registered by **[DEC-161]**. **[F06-R20]** (Must) requires each ledger row to link to its cause, and **[F06-R05]**/**[F06-R24]** (Must) require the actor to be visible; the DTO carries `causedByType`/`causedById` but no human reference (`TRD-`/`WDR-`/`DEP-`) and no `actorKind`. The redesigned ledger view links through the routes that already exist and infers the actor from the entry type until it lands |
| [OQ-112] | Registered by **[DEC-161]**. **[F06-R22]** (Should) wants the ledger exportable to CSV for a chosen period; only the PDF/statement export exists server-side. The redesigned Export panel pages through the ledger endpoint and assembles the CSV client-side until a server-side export lands |
