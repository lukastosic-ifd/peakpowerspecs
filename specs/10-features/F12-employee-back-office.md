...
| ID | Requirement | MoSCoW |
| --- | --- | :--: |
| F12-R39 | The customer detail page shows the company's **four-eyes mode** (on/off) and lists its **admin accounts** — the accounts that can approve or decline — alongside the ordinary account list **[F12-R14]**. ~~An account's **admin** flag is set and cleared here, by a PeakPower employee **[DEC-16]**, **[DEC-71]**.~~ ⚠ **Corrected 2026-09-10: there is no admin flag.** An account's **`membershipRole`** (`admin`, `trader` or `viewer`) is set and changed here instead, still by a PeakPower employee, through the **same edit** that changes name, job title, phone and email — not a separate control (`AccountEndpoints.cs:40`, the `ChangeRole` call at `:198` @ debadda0). ⚠ Unlike the customer's own `PATCH /company/memberships` [api-contracts §2.7], **this edit enforces no admin-floor check** — nothing here refuses a change that would leave a four-eyes company, or any company, with no admin. ⚠ **Amended 2026-09-23 by [DEC-157]:** the edit **now refuses a demotion that would drop the company below its admin floor** (two active admins for a four-eyes company, one otherwise), race-safe — the back-office twin of the customer `PATCH /company/memberships` and of the [DEC-154] remove-from-company floor; the "enforces no admin-floor check" statement is closed. Deactivating an admin below the floor is handled separately and is **warned, not refused** **[F12-R43]**. | Must |
