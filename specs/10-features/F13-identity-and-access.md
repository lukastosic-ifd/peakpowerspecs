...
| F13-R02 | ⚠ **Reversed for customers 2026-09-03 by [DEC-113], and for staff 2026-09-08 by [DEC-146].** There is no provider left to own a credential **[DEC-119]**, so the platform holds an **Argon2id** hash in **both** realms — on `customer.customer_account` **[DEC-113]** and on `employee.employee` **[DEC-137]**, **[DEC-142]** — and **owns the reset path on both sides**: `customer.password_reset_token` **[DEC-113]** and `employee.password_reset_token` **[DEC-146]**, single-use, one-hour, stored only as a hash. The requirement is kept rather than deleted because it is what has to be reinstated the day a provider exists, and because its **containment** is what survives and still binds: a hash is never logged and never returned by any endpoint, and every control the **[DEC-53]** exception demanded — hashing, rotation, revocation — now applies to both realms. ⚠ **What no longer follows from it:** the break-glass accounts of **[DEC-53]**, **[F13-R33..R40]**, are no longer the *only* platform-held credentials, and have not been since **[DEC-137]** gave the back office ordinary platform-held sign-in. Those requirements are unchanged and still unbuilt — a break-glass account is disabled by default, second-factored off-provider, alerted and rehearsed, none of which describes an everyday staff login. Original text: *"The platform never receives or stores a **customer** password. The identity provider owns the credential **[DEC-29]** — no credential storage, reset flow or lockout policy is built for customers. The **only** platform-held credentials anywhere in the system are the named employee break-glass accounts of **[DEC-53]**, **[F13-R33..R40]**."* | Must |
...
| F13-R10 | ⚠ **Reversed for customers 2026-09-03 by [DEC-113], and for staff 2026-09-08 by [DEC-146].** There is no provider to hand any of it to **[DEC-119]**. **Password reset is the platform's, in both realms**, by an emailed single-use token; failed sign-ins carry a **progressive delay** rather than a hard lockout, because a hard lockout on a username is a denial-of-service primitive aimed at a named account **[DEC-113]**. ⚠ **The last clause is the half that survives, and it is now an obligation on this platform rather than on a provider**: nothing may leak whether an account exists. The reset request answers `202` for a known and an unknown address alike; the known-address branch **swallows and logs its own database and mail failures**, so a `500` cannot mark a real address; the send is **enqueued, never awaited**, so a stopwatch cannot either **[DEC-133]**, **[DEC-146]**; the throttle counts every request, not only the ones that find somebody; and unknown, spent and expired codes are refused identically. Original text: *"Failed logins, lockouts and password resets are handled by the provider; the platform surfaces provider errors without leaking whether an account exists **[DEC-29]**."* | Must |
...
| ID | Requirement | MoSCoW |
| --- | --- | :--: |
| F13-R12 | Roles come from the token: `customer.user`, `employee.viewer`, `employee.trader`, `employee.finance`, `employee.admin` **[Actors & roles](../00-overview/03-actors-and-roles.md)**. ⚠ **Name collision resolved 2026-09-10 by [DEC-152], not reversed.** This vocabulary is the **employee** one and keeps every one of its names. A customer's membership role is a **separate** vocabulary — `admin`, `trader`, `viewer` on `customer.customer_membership` — which collides on `trader` and `viewer` and on nothing else. The collision is accepted rather than renamed away, because both spellings are the words the two populations actually use; the resolution is that **any code or document naming more than one spells all of them**: `employeeRole` for this row, **`membershipRole`** for the customer's, and `role`/job title for the descriptive field on the account record **[F01-R13]**. ⚠ **Also corrected 2026-09-03 by [DEC-119]**: the customer API carries no `roles` claim and no `customer.*` vocabulary at all **[F13-R43]** — and after **[DEC-152]** it carries no role claim of any kind, because the database is the sole authority on a membership role. ⚠ **Amended 2026-09-18 by [DEC-155]** — the **employee** realm now has a *built* role member, `is_admin` on `employee.employee`, and it too is **proven per request from the database, not carried in the token**: `EmployeeSessionMiddleware` records it and the `BackOfficeAdmin` policy reads that, so a demotion bites on the next request. The four `employee.*` names above stay the aspirational provider-era vocabulary; the built distinction is `is_admin` — **admin or ordinary**, not viewer/trader/finance/admin. | Must |
...
| F13-R41 | ⚠ **Reversed 2026-09-10 by [DEC-152] — both halves, and this is the requirement the design named as the F13 side of the reversal.** The flag itself is gone: `customer_account.is_admin` is **dropped** in migration 15 and the `is_admin` **claim leaves the token**, because the database is the sole authority on role. What replaces it is a **`membershipRole`** per business, so the same person can be `admin` in one and `viewer` in another. **First half reversed:** it is no longer *"set and cleared by a PeakPower employee, never by the customer"* — a customer admin sets the role of their own colleagues, which is the self-service **[DEC-16]** refused. **Second half reversed:** it no longer *"grants no additional read or write"* — `admin` gates invitations, role change, removal and entitlements **[DEC-150]**, so the rule that any endpoint gating an ordinary write on it is a defect is withdrawn with it. ⚠ **What survives, and is still binding:** the role is **never derived from a request parameter**, and it is **never defaulted on** — there is still no *"first account of a company is admin"* rule. That clause is why migration 15's first-admin repair **writes an `audit.audit_record` naming a migration actor for every promotion** instead of quietly defaulting; a business with no active account at all is listed in the migration output and left alone. ⚠ **Named, not merely permitted:** onboarding *does* create the signer's own membership as `admin` at sign-up, by design (`OnboardingService.cs:436-437` @ debadda0) — but the code's own comment there (`:430-435`) argues, and this row agrees, that it is not the forbidden rule: it is the one moment the platform knows it is creating a business and the person signing for it in the same breath, so nothing is inferred from account order after the fact. The forbidden rule is a standing promotion applied to an *existing* account later — which is exactly what migration 15's repair does, and why that repair is the one path required to audit each promotion (**[F13-R23]** shares this source). And `admin` still grants **no additional read** **[F01-R14]**. Original text: *"A customer account carries an **admin** boolean **[DEC-71]**. It is set and cleared by a PeakPower employee **[DEC-16]**, never by the customer, never derived from a request parameter and never defaulted on (no "first account of a company is admin" rule). It grants **no additional read or write**: an admin sees and does exactly what a non-admin sees and does, and a **non-admin may still accept an offer [DEC-18]**. Its only effect is to make the account eligible to give — or refuse — a four-eyes approval **[F13-R44]**. Any endpoint that gates an ordinary read or write on the flag is a defect (§3, rule 11)."* | Must |
...
| F13-R48 | ⚠ **New 2026-09-18 by [DEC-155].** The back office authorises **staff-account management** to an **admin operator** only. `employee.employee` carries an `is_admin` boolean, **proven per request from the database, never a token claim** — `EmployeeSessionMiddleware` records it and the `BackOfficeAdmin` policy reads it, so a demotion takes effect on the next request — and that policy gates the six `/api/v1/operators` endpoints (list, create-with-invite, edit = rename + promote/demote, deactivate, reactivate, resend-invite) **on top of** the back-office tenancy classification every employee endpoint already carries; **every existing employee endpoint stays open to any operator** and the gate is additive. A new operator is invited by an emailed **set-your-password** link that reuses the `employee.password_reset_token` path **[DEC-146]** at a **14-day** lifetime (the forgot-password token stays one hour) and is redeemed through the **existing** completion endpoint — no admin sets or sees a staff password, and **[DEC-142]** makes the username the address the invite is sent to. The back office keeps **at least one active admin**: a demotion or deactivation that would leave none is **refused**, race-safe under a `SELECT … FOR UPDATE` spanning the check through the save. `app_employee_role` may write only the **non-credential** columns of `employee.employee` — `display_name, is_admin, is_active, security_stamp` — **never `password_hash` or `username`**; the token, reset and refresh tables stay **owner-connection only**. | Must |
...
| F13-R22 | ⚠ **Reversed 2026-09-10 by [DEC-152]** — and already half-reversed before it: **[DEC-119]** removed the provider this row hands the credential to. The shipped accept has **three** paths behind one anonymous endpoint, and the `INVITED` → `ACTIVE` transition is only one of them (`InvitationEndpoints.cs` @ debadda0). **(i)** A known address whose login is already `ACTIVE` gets a **membership only** — no account-status change (`:453-472`, membership creator `:459`). **(ii)** A **pre-existing `INVITED` account** — for example one the back office created — is completed **in place** via `unfinished.CompleteInvitation(...)` (`:402-414`): this is the **only** real `INVITED` → `ACTIVE` transition left. **(iii)** An **unknown address** has its account created **directly** as `AccountStatus.Active` (`:417-432` — the code's own comment says so): there is no provider identity to "activate", so for this path the `INVITED` → `ACTIVE` lifecycle is carried instead by `customer_invitation.accepted_at` moving null → timestamp (`MarkAccepted`, `:472`), never by the account status. ⚠ **All three paths are anonymous and run on the owner connection** — the invitation token is the authorisation, not tenancy: a known-address accept cannot run authenticated, because `app.customer_id` would be the invitee's *current* business and the row being inserted is the very thing that would make the admin predicate true. Indistinguishability is required for the **issue** only (contract §11) — the accept's `400`/`409` responses are visible to the token holder alone. Original text: *"Accepting an invitation activates the identity in the provider, where the person sets their own credential, and links it to the platform account record. The account moves `INVITED` → `ACTIVE`."* | Must |
| F13-R23 | A company may have any number of accounts. Creating the second and subsequent accounts is the same operation as the first — there is no notion of a primary or owner account. ⚠ **Upheld 2026-09-10 by [DEC-152], and annotated because the surrounding requirements were not.** Membership introduces no owner: **owning a business is being an `admin` of it**, which is a role several people hold at once, not a single privileged seat. Membership is created by five paths as shipped, not by one uniform operation: onboarding sign-up makes the signer's own membership `admin` (`OnboardingService.cs:436-437` @ debadda0); invitation accept creates or restores one (`InvitationEndpoints.cs:459`); the back office creates one with the role named in its request (`AccountEndpoints.cs:125`); the demo seeder creates one per seeded person (`DemoDataSeeder.cs:183`); and migration 15's backfill promotes an existing account for a business left with none. None of the five designates a primary or owner account, and the only asymmetry anywhere is the **floor** (two admins for a four-eyes business, one otherwise), which is a minimum on a population, not a seat. ⚠ The account that *does* start as `admin` at creation is the one who signs up and creates the company — but the shipped code's own comment (`OnboardingService.cs:430-435`) argues this is not **[F13-R41]**'s forbidden standing "first account is admin" rule: it is the one moment the platform knows it is creating a company and the person who signed for it in the same breath, so nothing is inferred after the fact. Migration 15's first-admin repair is a separate, later case — a one-off data fix for a business left with none — and it **writes an audit record per promotion** precisely because a standing "first account is admin" rule is what **[F13-R41]** forbids. | Must |
...
| F13-R28 | An account holder can see the other accounts of their own company — name, job title, email, status — so they know who else can act **[DEC-62]**, **[F01-R21]**. Under **[DEC-16]** any colleague can already spend the company's money, so this discloses nothing an employee could not tell them on the phone. ⚠ **Amended 2026-08-19 by [DEC-71]**: the list also shows **who is an admin**. Without it, a user at a four-eyes company cannot tell who to ask for approval, and a 30-minute offer window **[DEC-111]** is not long enough to find out by phone. ⚠ **Amended again 2026-09-10 by [DEC-152].** The list becomes the **member-management surface**: it shows each colleague's `membershipRole` and, for an `admin`, offers role change and removal in place — `GET`/`PATCH`/`DELETE /api/v1/company/memberships`. The *read* stays available to every member, unchanged and un-gated, because knowing who else can act is not a privilege. ⚠ **It is scoped to the active business and to nothing else, and the reason is sharper than privacy.** The membership policy's `USING` carries a permissive `OR` on `account_id` that makes the **querying account's own** memberships in **other** businesses technically readable while it acts for this one — not a colleague's: the `account_id` branch compares against the caller's own id, so no other account's row ever satisfies it, and a genuinely different colleague's membership at some third business stays invisible under any query this endpoint could write. So every admin query carries an explicit `AND customer_id = @active`, and what it actually guards is the query, not a third party's secrecy: left unfiltered, an admin's own membership in an unrelated business would leak into what is supposed to be *this* business's member list — and, load-bearing, into the **four-eyes admin floor count**, which an admin could otherwise inflate by self-registering a shell business where she is sole admin, so the count reads two where the real company has one, then removing the other admin unchallenged. **[DEC-152]**'s own rationale still describes this as showing "colleagues' **other** businesses" and carries whether to change it as registered debt; that phrasing is loose — it is always the acting account's own row the `OR` exposes, never a different colleague's — but the debt itself, and any change to the predicate, stays open there, not here. ⚠ **An earlier revision of the design attributed [F01-R21]'s "read-only; changes go through PeakPower" wording to this row. It does not carry that text** — this row was never read-only-by-wording, only by consequence. | Must |
...
- ~~**Any intra-company role beyond the admin flag** — a viewer / trader / approver hierarchy stays out
  ...
  **[OQ-105]**. ⚠ **Amended 2026-09-23 by [DEC-163]** — one behavioural difference now exists:
