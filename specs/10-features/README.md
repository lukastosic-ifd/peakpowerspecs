...
| F04 | **[OQ-23]** ⏸ | Blocking a real feed (the **[DEC-159]** fixture still renders the board): the 24 Montel ticker symbols (widened 2026-09-23 by **[DEC-161]** from six) were never supplied, so `montel_ticker` is empty for all 24 product rows. It also carries the **bid-vs-ask** wording — **[DEC-80]**'s two sources disagree, the comment says *bid* and governs — plus the sign convention for a negative quote and far-offset licence/service coverage, all to be confirmed together |
