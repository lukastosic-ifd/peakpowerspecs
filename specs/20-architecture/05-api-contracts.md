...
| `GET` | `/wallet/deposits/{id}` | ⚠ **Amended 2026-09-23 [DEC-159]** — any member, tenant-scoped **[F07-R11]** — the intent, its credited entries **[F07-R26]**, and **`expiresAt`** for an iDEAL intent **[DEC-159]**; for a bank transfer, **`instructions { iban, accountHolder }`** — the BIC is deferred **[OQ-108]**. **404** for another company's id or an unknown one, byte-identical either way |
...
| `GET` | `/wallet/payments/{id}` | Payment status. Retained: it is the PSP leg, not the deposit intent. ⚠ **Amended 2026-09-23 [DEC-159]/[DEC-161]** — no longer what the portal polls after the return: the return target moved to `/wallet/deposits/{id}` **[F07-R04]**, and **[F07-R09]**'s confirmation poll targets the `GET /wallet/deposits/{id}` row above |
