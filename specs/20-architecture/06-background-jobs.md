...
| `PollMontelIndicationsJob` | every 5 min (market hours), hourly otherwise | `integration` | Price indications, through the **existing Montel service [DEC-96]**, **[F08-R18]**. The **[DEC-80]** markup — a configurable percentage, default 2% — is applied at the presentation edge, never stored by this job **[F08-R17]**. ⚠ **As-built 2026-09-24**: this name and queue are the pre-build sketch. The built poller is `ForwardPricePollJob`, run by the `ForwardPriceScheduleHost` `BackgroundService` on a one-minute `PeriodicTimer` **inside the Worker process** — it is never enqueued to Hangfire's `integration` queue at all (see [Montel integration](../30-integrations/02-montel-api.md) §4) |
