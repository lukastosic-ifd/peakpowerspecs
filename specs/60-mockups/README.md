...
| [price-indications.svg](price-indications.svg) | [F04](../10-features/F04-price-indications.md) | Month/Quarter/Year selector; a Base/Peak forward-curve chart, delivery periods on the x-axis, with stale-point and unavailable-gap styling; a list beneath it with per-cell status; the **"Indication — not an offer"** label beside both the chart and the list, repeated in the chart's tooltip; an info-glyph disclaimer; and the sample-data note. Tiles are retired **[DEC-160]**; depth is 24 products **[DEC-161]** |
